...
async function collectApple(query) {
  ...
  for (const item of (data.results || [])) {
    add({
      ...
      rating: numeric(item.averageUserRating),
      rating_count: numeric(item.userRatingCount),
      downloads: null,
...
async function collectGoogle(query) {
  ...
  for (const item of items) {
    add({
      ...
      description: clean(item.summary || item.description),
...
async function save() {
  const apps = [...records.values()].map(finalize).sort((a, b) => a.store - b.store || a.name.localeCompare(b.name));
